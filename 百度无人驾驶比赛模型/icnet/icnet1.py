...
def shared_convs(image):
    tmp = conv(image, 3, 3, 32, 1, 1, padding='SAME', name="conv1_1_3_3_s2")
    ...
    tmp = bn(tmp, relu=True)
    tmp = max_pool(tmp, 3, 3, 1, 1, padding=[1, 1])
...
def sub_net_4(input, input_shape):
    tmp = interp(input, out_shape=np.ceil(input_shape // 32))#32
...
def sub_net_1(input):
    ...
    tmp = bn(tmp, relu=True)
    tmp = conv(tmp, 3, 3, 32, 1, 1, padding='SAME', name="conv2_sub1")
    tmp = bn(tmp, relu=True)
    tmp = conv(tmp, 3, 3, 64, 1, 1, padding='SAME', name="conv3_sub1")
    tmp = bn(tmp, relu=True)
...
def CCF124(sub1_out, sub24_out, input_shape):
    ...
    tmp = tmp + sub1_out
    tmp = fluid.layers.relu(tmp)
    tmp = interp(tmp, input_shape // 1)
    return tmp


def icnet(data, num_classes, input_shape):
    image_sub1 = data
    image_sub2 = interp(data, out_shape=input_shape * 0.5)#512

    s_convs = shared_convs(image_sub2)#256
    s_convs = dropout(s_convs, dropout_keep_prop)
    #print('s_convs', s_convs)
    sub4_out = sub_net_4(s_convs, input_shape)#256
    # sub4_out = dropout(sub4_out, dropout_keep_prop)
    sub2_out = sub_net_2(s_convs)
    sub1_out = sub_net_1(image_sub1)
    # sub1_out = dropout(sub1_out, dropout_keep_prop)
    #print('sub1_out',sub1_out)
    sub24_out = CCF24(sub2_out, sub4_out, input_shape)
    sub24_out = dropout(sub24_out, dropout_keep_prop)
   # print('sub24_out', sub24_out)
    sub124_out = CCF124(sub1_out, sub24_out, input_shape)
    sub124_out = dropout(sub124_out, dropout_keep_prop)
